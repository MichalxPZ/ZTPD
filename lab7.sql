...
from STREETS_AND_RAILROADS st, MAJOR_CITIES c
WHERE SDO_RELATE(st.GEOM,
...
SELECT 'A6_LRS', 'GEOM', T.DIMINFO, T.SRID
FROM ALL_SDO_GEOM_METADATA T
WHERE T.TABLE_NAME = 'A6_LRS';
...
SELECT SDO_LRS.GEOM_SEGMENT_LENGTH(SDO_LRS.CLIP_GEOM_SEGMENT(A6.GEOM, 50, 200, 0)) / 1000 koszt_w_mln
FROM A6_LRS A6;
